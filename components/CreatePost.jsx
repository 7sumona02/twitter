...
const CreatePost = () => {
    ...
    if (loading) return (
        <div className={`flex flex-col space-y-3 ${isPending ? 'opacity-50' : 'opacity-100'}`}>
    ...
    return (
        <div className="flex flex-col justify-start mt-5 border-b pb-5 w-[55.4vw] pl-20 pr-5 space-y-5">
            ...
            <div className='w-full flex justify-between items-center relative'>
                ...
                <div>{isDisabled ? <Button className='cursor-pointer select-none' disabled>Post</Button> : <Button onClick={PostTweet} className='cursor-pointer select-none'>Post</Button>}</div>
            </div>
        </div>
    )
}
